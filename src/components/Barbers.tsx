...
export default function Barbers() {
  ...
  const renderBarberCard = (b: typeof barbers[0], extraClass = '') => (
    <div
      key={b.id}
      id={`barber-${b.id}`}
      className={`glass-card rounded-2xl overflow-hidden border border-gold/10 hover:border-gold/30 hover:-translate-y-2 transition-all duration-400 group flex flex-col${extraClass ? ` ${extraClass}` : ''}`}
    >
      {/* Photo */}
  ...
  return (
    <section id="equipo" className="w-full pt-32 pb-24 bg-dark">
      <div className="w-full max-w-screen-2xl mx-auto px-6">
        ...
        {/* Filas completas — grid 4 columnas */}
        {fullRows.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {fullRows.map((b) => renderBarberCard(b))}
          </div>
        )}
